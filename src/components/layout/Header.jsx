...
import { Heart, ShoppingCart, User, ChevronDown, LogOut, Package, MapPin, Settings, Star, Bell, HelpCircle, LayoutDashboard, Menu, X } from 'lucide-react';
  ...
  return (
    <>
      <header
        ...
        style={{ backdropFilter: scrolled ? 'blur(12px)' : 'none', backgroundColor: scrolled ? 'rgba(255,255,255,0.97)' : '#fff' }}>
        ...
        <div className="max-w-[1400px] mx-auto px-3 md:px-4 py-2.5 md:py-3 flex items-center gap-2 md:gap-4">
          ...
          <div className="flex items-center gap-2 md:gap-4 ml-auto md:ml-0">
            {/* Wishlist */}
            <Link to="/account/wishlist" className="relative flex flex-col items-center text-gray-600 hover:text-[#FF5A1F] transition-colors group">
              ...
              <span className="hidden lg:block text-[10px] mt-0.5">Wishlist</span>
            </Link>

            {/* Cart */}
            <Link to="/cart" className="relative flex flex-col items-center text-gray-600 hover:text-[#FF5A1F] transition-colors group">
            ...
            <div className="relative hidden md:block" ref={profileRef}>
                ...
                className="flex items-center gap-2 text-gray-700 hover:text-[#FF5A1F] transition-colors">
                   ...
                   <span className="text-[#FF5A1F] font-bold text-sm">{(user?.name || user?.firstName || 'U')[0].toUpperCase()}</span>
                   }
                   </div>
                   <span className="hidden lg:block text-sm font-medium">Hi, {user?.name?.split(' ')[0] || user?.firstName || 'User'}</span>
                   <ChevronDown size={14} className={`hidden lg:block transition-transform ${profileOpen ? 'rotate-180' : ''}`} />
                 </button>
                ...
                <motion.div
                  ...
                  className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-50">
                      ...
                      <div className="px-4 py-3 bg-orange-50 border-b border-orange-100">
                        <p className="text-sm font-semibold text-[#111827]">{user?.name || user?.firstName}</p>
            ...
            <Link to="/login" className="hidden md:flex flex-col items-center text-gray-600 hover:text-[#FF5A1F] transition-colors">
                <User size={22} />
                <span className="hidden lg:block text-[10px] mt-0.5">Login</span>
              </Link>
            }
      ...
      <AnimatePresence>
        ...
        <>
            ...
            <div className="p-4 border-b">
                  <Link to="/login" onClick={() => setMobileMenuOpen(false)} className="btn-primary w-full justify-center">Login / Register</Link>
                </div>
            }
              ...
              className="block px-3 py-2.5 text-sm text-gray-700 hover:bg-orange-50 hover:text-[#FF5A1F] rounded-lg transition-colors">
                ...
                {isAuthenticated &&
              <button onClick={() => {setMobileMenuOpen(false);setLogoutModal(true);}}
        ...
        <motion.div
          ...
          className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            ...
            className="bg-white rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl">
              ...
              <h3 className="text-xl font-bold text-[#111827] mb-2">Logout from Shopy?</h3>
              <p className="text-gray-500 text-sm mb-6">Are you sure you want to logout from your account? You will need to login again to access your account.</p>
              <div className="flex gap-3">
                <button onClick={() => setLogoutModal(false)} className="flex-1 btn-outline">Cancel</button>
